...
@Component({
  ...
  templateUrl: 'EventClaimsDetails.component.html',
  styleUrls: ['EventClaimsDetails.component.scss']
})

export class EventClaimsDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.eventClaimService.getEventClaimDetails(claimId).subscribe((data) => {
  ...
  loadFiles(claimId : number) {
    this.eventClaimService.getFilesByEventClaimId(claimId).subscribe((data : File[]) => {
      this.files = data;
